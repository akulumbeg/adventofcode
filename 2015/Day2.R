...
# wrapping paper for each gift a little easier: 
...
rm(list = ls()) # Initialize workspace

# Prepare data as a dataframe 1000x3
gifts <- as.data.frame(readr::read_lines("2015/data/Day2.txt"), stringsAsFactors = F)
names(gifts)[1] <- "gift"
gifts <- gifts %>% tidyr::separate(gift, c("l", "w", "h"), sep = "x")
gifts <- sapply(gifts[,1:3], as.integer)
gifts <- as.data.frame(gifts)

# Wrapping paper calculation functions
wrapping_paper <- function(x) {
  sum <- 0
  for (i in 1:nrow(x)) {
    sum <- sum + (2*x[i,1]*x[i,2] + 
                    2*x[i,2]*x[i,3] + 
                    2*x[i,3]*x[i,1] + 
                    min(x[i,1]*x[i,2],
                        x[i,2]*x[i,3],
...
ribbon <- function(x) {
  sum <- 0
  for (i in 1:nrow(x)) {
    ribbon_1 <- sum(sort(x[i,])[1:2]*2) 
    ribbon_2 <- x[i,1] * x[i,2] * x[i,3]
    sum <- sum + ribbon_1 + ribbon_2
  }
  return(sum)
}
